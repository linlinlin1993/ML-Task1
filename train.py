...
#Prepare input data
classes= list()
for i in range(len(idx2car)):
    key=i+1
    classes.append(idx2car[key])
#print(classes)
...
# We shall load all the training and validation images and 
#labels into memory using openCV and use that during training
data=dataset.read_train_sets(train_path,img_size,classes, validation_size=validation_size)

print("Complete reading input data.")
print("Number of files in Traning.set:\t\t{}".format(len(data.train.labels)))
...
def create_convolutional_layer(input,num_input_channels,conv_filter_size,num_filters):
    ...
    weights=creat_weights(shape=[conv_filter_size, conv_filter_size, num_input_channels, num_filters])
    ##create biases
...
def create_flatten_layer(layer):
    ...
    ##Number of features will be img_height*img_wigth*num_channels
...
def train(num_iteration):
    ...
    for i in range(total_iterations,
        ...
        if i % int(data.train.num_examples/batch_size) == 0: 
            ...
            saver.save(session, './model',global_step=i) 
